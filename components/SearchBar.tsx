      ...
      <DialogContent className='max-w-5xl min-h-[90vh] max-h-[90vh] flex flex-col overflow-hidden' >
        <DialogHeader>
          ...
          <form className="relative" onSubmit={(e) => e.preventDefault()}>
            ...
            />
            {search && (
              <X onClick={()=> setSearch("")} className="w-4 h-4 absolute top-3 right-11 hover:text-red-600 hoverEffect"/>
            )}
            <button 
            type='submit' 
              ...
              products?.map((product: Product)=> (
                <div key={product?._id} className='bg-white overflow-hidden border-b last:border-b-0 '>
                  <div className='flex items-center p-1'>
                    <Link href={`/product/${product?.slug?.current}`} className='h-20 w-20 md:h-24 flex-shrink-0 border border-background/20 rounded-md overflow-hidden group'
                  ...
                  <div className='px-4 py-2 flex-grow'>
                    <Link href={`/product/${product?.slug?.current}`} onClick={()=> setShowSearch(false)}>
                      ...
                      <p className='text-sm text-gray-600 line-clamp-1'>{product?.intro}
            ...
            <div className='text-center py-10 font-semibold tracking-wide'>
              {search && !loading ? (
                ...
                <p className='text-green-600 flex items-center justify-center gap-1'>
                  <Search />
                  Search and explore your products from Tuvi
                </p>
              )}
            </div> 
            )}
